---
title: "Parental Deaths From Drugs and Firearms Combined"
author: "Mathew Kiang"
date: "`r Sys.Date()`"
output: 
  html_document:
...
per1000_loss <-
    ...
    mutate(
        drugs = sprintf(
            "%0.2f (%0.2f to %0.2f)",
            round(median_drugs  * 1000, 2),
            round(lower95_drugs * 1000, 2),
            round(upper95_drugs * 1000, 2)
        ),
        firearms = sprintf(
            "%0.2f (%0.2f to %0.2f)",
            round(median_firearms  * 1000, 2),
            round(lower95_firearms * 1000, 2),
            round(upper95_firearms * 1000, 2)
        )
    ) |>
    dplyr::select(race, year, drugs, firearms) |>
    pivot_wider(names_from = year,
                values_from = drugs:firearms,
                names_prefix = "per1000_loss_") |>
    categorize_race()

## per 1000 kids, drugs and firearms combined
per1000_loss_both <-
    df.inc.parental.loss.by.cause |>
    ungroup() |>
    filter(cause != "other",
...
cum_n_loss <-
    ...
    dplyr::select(race, drugs_cume_n_loss, firearms_cume_n_loss) |>
     categorize_race()
...
cum_n_loss_both <-
    ...
    dplyr::select(race, both_cume_n_loss = both) |> 
    categorize_race()
...
cum_n_loss_other <-
    ...
    dplyr::select(race, other_cume_n_loss = other) |> 
    categorize_race()
...
cum_n_loss_both_per1000 <-
    ...
    dplyr::select(race, both_cume_n_loss_per1000 = both) |>
    categorize_race()
...
cum_n_loss_other_per1000 <-
    ...
    dplyr::select(race, other_cume_n_loss_per1000 = other) |>
    categorize_race()
...
perc_inc_n_loss <-
    ...
    dplyr::select(cause, race, contains("perc_in")) |>
    pivot_wider(names_from = cause,
    ...
    dplyr::select(race, drugs_perc_inc, firearms_perc_inc) |>
    categorize_race()
...
perc_inc_n_loss_both <-
    ...
    dplyr::select(race, contains("perc_in")) |>
    ...
    dplyr::select(race, both_perc_inc) |>
    categorize_race()
...
perc_inc_n_loss_other <-
    ...
    dplyr::select(race, other_perc_inc) |>
    categorize_race()
...
abs_inc_n_loss <-
    ...
    dplyr::select(cause, race, contains("abs_in")) |>
    pivot_wider(names_from = cause,
    ...
    dplyr::select(race, drugs_abs_inc, firearms_abs_inc) |>
    categorize_race()
...
abs_inc_n_loss_both <-
    ...
    dplyr::select(race, contains("abs_in")) |>
    ...
    dplyr::select(race, both_abs_inc) |>
    categorize_race()
...
abs_inc_n_loss_other <-
    ...
    dplyr::select(race, other_abs_inc) |>
    categorize_race()
...
## Table S2
```{r}
print_table <- wide_table |>
    transmute(
        cause_of_death = "both",
        race_cat,
        n_1999 = both_n_1999,
        pop_1999,
        per1000_loss_1999 = both_per1000_loss_1999,
        n_2020 = both_n_2020,
        pop_2020,
        per1000_loss_2020 = both_per1000_loss_2020,
        abs_inc = both_abs_inc,
        perc_inc = both_perc_inc,
        cume_n_loss = both_cume_n_loss,
        cume_n_per1000 = both_cume_n_loss_per1000
    )
...
print_table |>
    ...
    kableExtra::pack_rows(
        "Estimated youth impacted by parental deaths due to fatal drug poisonings and firearms combined", 1, 4)
```
```{r}
write_csv(print_table,
          here("outputs", "tables_data", "tableS2_data_both.csv"))
```